...
public enum AppReadModeState {
    ...
    case EndState//stop, this is when touch pointer is not touched
    case FeedbackState//feedback when at end of the page
}

public enum AppReadModeEvent  {
    case Start//go to start when initialised
...
extension AppReadModeState: DOTLabelable {
    ...
    {
        return [ .InitialState, .ReadingState, .PauseState, .EndState, .FeedbackState]
    }
...
extension AppReadModeEvent: DOTLabelable 
{
    ...
    {
        return [.Start, .ActivateReading, .Failed, .Pause, .End, .ActivateFeedback]
    }
